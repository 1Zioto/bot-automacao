...
const { hashSenha, verificarSenha, gerarToken, usuarioDoReq } = require('../_auth');
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (acao !== 'impersonate' && (!email || !senha)) {
            return json(res, 400, { erro: 'Informe email e senha.' });
        }
        if (acao === 'impersonate' && !email) {
            return json(res, 400, { erro: 'Informe email.' });
        }
        ...
        if (acao === 'impersonate') {
            const adminUser = usuarioDoReq(req);
            if (!adminUser || adminUser.email !== 'douglaszioto@gmail.com') {
                return json(res, 403, { erro: 'Acesso negado. Apenas o super admin pode fazer isso.' });
            }
            const r = await query('SELECT id, email, nome FROM usuarios WHERE email = $1', [email]);
            const u = r.rows[0];
